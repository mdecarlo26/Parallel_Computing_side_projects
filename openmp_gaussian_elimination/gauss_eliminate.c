...
void gauss_eliminate_using_omp(Matrix inputMat,int thread_cnt)
{
    int i, j, k;
    unsigned int num_elements = inputMat.num_rows;
    float* U = inputMat.elements;

    #pragma omp parallel num_threads(thread_cnt) private(i,j,k)
    {
